<template>
  <div class="Loginbox">
    <div class="boxchild">
      <div class="Logintitle">Register Page</div>
...
export default {
  ...
  methods: {
    userregister() {
      if (this.users.name == "" || this.users.password == "") {
        ...
        axios.post("/api/user/register", this.users).then(
          (Response) => {
            window.alert(Response.data, "请登录");
  ...
  mounted() {
    if (this.$store.state.state) {
      this.$router.push("/"), window.alert("请退出登陆后注册");
    }
  },
};
</script>
